    ...
    for entry in blocks {
        ...
        for state in states {
            ...
            for property in properties {
                let pascal_case = util::namespace_to_pascal_case(property.0);
                blockstate_entry += format!("{}: crate::blocks::{}::{}::{}", pascal_case, file_name, pascal_case, util::property_instance_to_rust_identifier(property.1.as_str().unwrap())).as_str();
            }
            blockstate_entry += "}";
