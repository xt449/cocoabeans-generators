...
pub fn generate() {
    ...
    fs::create_dir_all("./blocks/src/")
        .expect("Unable to write to output location './blocks/src/'!");
    ...
    for entry in blocks {
        ...
        for state in states {
            ...
            if state_properties_raw.is_none() {
                ...
                blockstate_registry.push((
                    format!("crate::block_state::{} {{}}", pascal_name),
        ...
        match object.get("properties") {
            None => {
                ...
                // Simple block state
                blockstate_struct =
                    format!("#[derive(Eq, PartialEq)]pub struct {} {{}}", pascal_name);
            ...
            Some(properties) => {
                ...
                blockstate_struct = format!(
                    "#[derive(Eq, PartialEq)]pub struct {} {{ {} }}",
                    pascal_name,
                    block_property_names
                        .iter()
                        .map(|p| format!(
                            "pub {}: {}{},",
                            util::property_instance_to_rust_identifier(p),
                            pascal_name,
                            util::namespace_to_pascal_case(p)
                        ))
                        .collect::<String>()
                );
                // set block state stringer
        ...
        let default_impl: String = format!(
            ...
            pascal_name, block_default_state
        );
        let blockstate_impl: String = format!(
            "impl BlockState for {} {{ {} }}",
            pascal_name, blockstate_stringer
        );
